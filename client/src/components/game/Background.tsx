...
export default function Background({ width, height }: BackgroundProps) {
  ...
  const drawClouds = (ctx: CanvasRenderingContext2D) => {
    ...
    // Further reduced cloud widths to make them narrower (additional 20% reduction from original)
    const cloudLayers = [
      { y: 40, width: width * 0.24, height: height * 0.15, speed: 1.0, offsetMultiplier: 1.0 },
      { y: 80, width: width * 0.16, height: height * 0.1, speed: 1.2, offsetMultiplier: 1.2 },
      { y: 20, width: width * 0.20, height: height * 0.12, speed: 0.8, offsetMultiplier: 0.8 }
    ];
